...
const readTemplate = (inputTemplateFile) => {

    return fs.readFileSync(inputTemplateFile, { encoding: 'utf8' });
}

// Render the Mustache template.
const renderTemplate = (template, content, partials) => {

    return Mustache.render(template, content, partials);
}

// Validate a terminology structure using the FHIR® JSON schema.
const validateTerminologyStructure = (terminologyStructure) => {

    // Load the FHIR® JSON schema for validation.
    const fhirSchema = fs.readFileSync('./schemas/R5/fhir.schema.json', { encoding: 'utf8' });
...
// Load a CodeSystem into the memory map.
const loadCodeSystemIntoManyKeysMap = (codeSystemId, codeSystemMetaVersionId, concept) => {
...
const loadCodeSystemsFromDisk = () => {
    ...
    files.forEach(file => {
        ...
        loadCodeSystemIntoManyKeysMap(codeSystem.id, codeSystem.meta.versionId, codeSystem.concept);
